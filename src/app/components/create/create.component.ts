...
export class CreateComponent implements OnInit {
    ...
    constructor(private gameservice: GameService, private fb: FormBuilder, private router: Router, private electronService: ElectronService, private ref: ChangeDetectorRef) {
        this.createForm();
    }
    ...
    saveImage() {
        if (this.enabled) {
           // WebCamera.snap(function (data_uri) {
               console.log("checking image" , this.image)
                var imageBuffer = this.image.match(/^data:([A-Za-z-+\/]+);base64,(.+)$/), response = <any>{};
		 ...
		 takesnapshot() {
			...
			WebCamera.snap( (data_uri) => {
                ...
                console.log("takesnap" , this.image)
                this.image =  data_uri;
                console.log("takesnap" , this.image)
				document.getElementById('results').innerHTML = 
					'<img src="'+data_uri+'"/>';
